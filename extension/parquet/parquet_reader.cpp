...
AsyncResult ParquetReader::ScanDirectDoublesToSink(ClientContext &context, ParquetReaderScanState &state,
                                                   idx_t output_count, idx_t capacity, idx_t &rows_out,
	...
	if (filters) {
		throw InvalidInputException("direct double parquet sink scan does not support table filters");
	}
	auto &deletion_filter = state.root_reader->Reader().deletion_filter;
	if (deletion_filter) {
		throw InvalidInputException("direct double parquet sink scan does not support deletion filters");
	}
	if (state.finished) {
		return SourceResultType::FINISHED;
	}
